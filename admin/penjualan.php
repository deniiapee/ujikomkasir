    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        // Retrieve selected product IDs
    $selected_products = isset($_POST['nama_barang']) ? $_POST['barang'] : array();
    exit();
   // Lakukan pengurangan stok barang
foreach($selected_products as $produk_id) {
    ...
    $sql = "SELECT stock FROM produk WHERE id = ?";
    ...
    if ($stmt->num_rows > 0) {
        $stmt->bind_result($stock_sekarang);
        $stmt->fetch();
        ...
        $sql_update = "UPDATE produk SET stock = ? WHERE id = ?";
        $stmt_update = $mysqli->prepare($sql_update);
        $stmt_update->bind_param("ii", $stock_sekarang, $produk_id);
        $stmt_update->execute();
    }
